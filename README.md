...
| **408 刷题** | 导航栏最右新增。刷题库总览（已刷题数 / 正确率 / 错题本 / 覆盖章节）+ 三科卡片，点击即在站内打开原刷题本；主题自动适配主站的粉色系 |
...
├── 408刷题/              # 三科刷题本（原样保留，作为站内 iframe 的来源）
│   ├── index.html              # 原入口页（站内不再使用，可保留作独立访问）
│   ├── 操作系统刷题本.html      # 650 题
│   ├── 计组刷题本.html          # 604 题
│   └── 计网刷题本.html          # 564 题
├── preview/            # 各页面效果截图
...
**关于「学习日」的 4 点规则**：墨墨以**凌晨 4 点**为界 —— 0:00~4:00 背的单词仍算前一天。
脚本已按此规则自动归类（把当前时间往前推 4 小时再取日期），所以凌晨 02:30 抓到的数据会正确写进**前一天**。

手动补录指定学习日（例如 4 点前又多背了几页）：

```bash
python update.py --memo --today 2026-09-09     # 指定写入哪个学习日
python tools/memo.py --day 2026-09-09          # 只看一眼，不写入
```

> ⚠️ **过了凌晨 4 点就无法再补前一天的数据了。** 墨墨开放 API 只返回「当前学习日」的进度，
> 不提供历史某天的完成量 —— 所以补录必须在 4 点前完成。这也是自动任务设在 02:30 的原因。
...
导航栏最右的「408 刷题」把三份原本独立的刷题本接进了看板。

### 它做了什么

| 层面 | 说明 |
| --- | --- |
| 总览页 | 四张统计卡（已刷 / 正确率 / 错题本 / 题库总量）+ **刷题强度热力图** + **近 100 题正确率** + 三科卡片 |
| 站内刷题 | 点科目卡片 → 同源 iframe 打开原刷题本，配「刷新」「新窗口」按钮 |
| 主题适配 | 注入主站配色：深色模式转深色、浅色模式白底 + 粉色主色 |

### 刷题强度热力图（近 18 周）

GitHub 贡献图样式：每格一天，颜色深浅按**当天三科作答总数**分七档：

```
0 题 · 1–19 · 20–29 · 30–39 · 40–49 · 50–59 · ≥ 60
```

悬浮任意一格会显示当天各科明细。**题库原始记录不带时间戳**，历史无法回溯，
所以热力图从上线当天开始按天记账（存在 `qz-log-v1`，凌晨 4 点前算前一天，与墨墨同口径）。

### 近 100 题正确率

三科各自取**最近 100 次作答**算正确率，并与历史累计对比：

- 绿色 `+x%`：近 100 题比历史平均好，手感在回升
- 红色 `−x%`：低于历史平均，该回炉错题本了

样本不足 100 题时按已有次数计算，卡片下会标注「已记录 N 次作答」。

### 为什么用 iframe 而不是重写

三份刷题本各含 560~650 道题（单个文件 300~400KB），功能已经打磨完整
（错题本、组卷测试、图片、行内备注、单题编辑、Gist 云同步）。
重写等于把这些推倒重来，风险高、收益低。iframe 能**零改动保留全部功能**，
只在其上叠加一层主题 —— 是本需求下性价比最高的做法。

### 进度统计怎么来的

刷题本把进度存在 localStorage，主站直接读同一份数据：
...
结构：`{ [题目id]: {a:答题次数, w:错次, l:最近是否错, b:是否在错题本} }`

主站据此算出「已刷题数（a>0）」「错题本（b===true）」「正确率（(Σa−Σw)/Σa）」。
**主站只读不写**，不会干扰刷题本自身逻辑。

### 两点要注意

1. 进度存在**本机浏览器**里。换设备、换浏览器、清缓存都会重置 ——
   需要迁移就用刷题本自带的「导出 JSON / 导入」，或配置它的 GitHub Gist 云同步。
2. 如果之前是在 `file://` 下（双击本地文件）打开的刷题本，那批数据与网页版**不互通**
   （`file://` 与 `https://` 是不同源），需要用「导出 JSON → 网页版导入」迁移一次。

> ⚠️ **版权提醒**：`408刷题/` 内是王道考研题库，仓库若为 Public 则内容对所有人可见。
> 仅供个人备考使用；若在意，可把仓库转为 Private（注意 GitHub Pages 对 Private 仓库需要 Pro）。
